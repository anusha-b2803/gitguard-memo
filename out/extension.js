...
function activate(context) {
    ...
    // Register Commands
    context.subscriptions.push(vscode.commands.registerCommand('gitguard.addToMemo', async () => {
        ...
        try {
            ...
            const autoDelete = vscode.workspace.getConfiguration('gitguard').get('autoDeleteMemosAfterCommit', false);
            if (autoDelete) {
    ...
    }), vscode.commands.registerCommand('gitguard.editMemoItem', async (item) => {
    ...
    }), vscode.commands.registerCommand('gitguard.moveMemoUp', (item) => {
        memoManager.moveItem(item.entry.id, 'up');
        memoTreeProvider.refresh();
    }), vscode.commands.registerCommand('gitguard.moveMemoDown', (item) => {
        memoManager.moveItem(item.entry.id, 'down');
        memoTreeProvider.refresh();
    }), vscode.commands.registerCommand('gitguard.toggleMemoPin', (item) => {
        const activeRepo = branchWatcher.getActiveRepository();
...
class BranchWatcher {
    constructor(context) {
        ...
        this._onDidBranchChange = new vscode.EventEmitter();
        this.onDidBranchChange = this._onDidBranchChange.event;
        const gitExtension = vscode.extensions.getExtension('vscode.git');
        if (gitExtension) {
    ...
    async configure(context) {
        ...
        const mode = vscode.workspace.getConfiguration('gitguard').get('storageMode') || 'global';
    ...
    async updateColors() {
        ...
        if (this.isCritical(branchName)) {
            ...
            if (!ruleMatches) {
                const newRule = { pattern: `^${branchName}$`, backgroundColor: hex, foregroundColor: fg };
                const updatedRules = [...rules];
                if (existingIdx !== -1)
                    updatedRules[existingIdx] = newRule;
                else
                    updatedRules.push(newRule);
                const mode = config.get('storageMode') || 'global';
                const target = (mode === 'workspace' && vscode.workspace.workspaceFolders) ?
                    vscode.ConfigurationTarget.Workspace : vscode.ConfigurationTarget.Global;
    ...
    isRepositoryIgnored(repoPath) {
        const ignored = vscode.workspace.getConfiguration('gitguard').get('ignoredRepositories') || [];
    ...
    async clearGlobalOrphans(allKeys) {
        const config = vscode.workspace.getConfiguration('workbench');
        const inspect = config.inspect('colorCustomizations');
    ...
    async repairGitWorkflow(repoRoot) {
        const confirm = await vscode.window.showInformationMessage('Repairing Git Workflow will untrack .vscode folder and ignore local settings. This fixes "Checkout Overwritten" errors. Proceed?', { modal: true }, 'Yes', 'No');
        if (confirm !== 'Yes')
            return;
        // 1. Untrack from index
        (0, child_process_1.exec)('git rm -r --cached --sparse .vscode', { cwd: repoRoot }, (err) => {
            if (err) {
                // Ignore errors if already untracked
                console.log('[GitGuard] rm --cached failed (might already be untracked).');
            }
            // 2. Skip worktree
            (0, child_process_1.exec)('git update-index --skip-worktree .vscode/settings.json', { cwd: repoRoot }, (innerErr) => {
                // 3. Update .gitignore
    ...
    async ensureGitIgnore(repoRoot) {
        try {
    ...
    async skipWorktree(repoRoot) {
        ...
        (0, child_process_1.exec)(`git update-index --skip-worktree ${settingsPath}`, { cwd: repoRoot }, (err) => {
            if (!err) {
    ...
    async ensureLocalExclude(repoRoot) {
        try {
            const excludePath = path.join(repoRoot, '.git', 'info', 'exclude');
            if (!fs.existsSync(path.dirname(excludePath)))
                return; // Not a standard Git repo or no info dir
    ...
    async resetColors() {
        ...
        if (inspect?.workspaceValue) {
            ...
            for (const key of allKeys) {
                if (key in wsCustoms) {
                    delete wsCustoms[key];
                    changed = true;
                }
            }
            if (changed)
                await config.update('colorCustomizations', wsCustoms, vscode.ConfigurationTarget.Workspace);
        }
        // Clean Global
        if (inspect?.globalValue) {
            const gCustoms = { ...inspect.globalValue };
...
class MemoManager {
    ...
    togglePin(id, currentBranchName) {
        const memoBuffer = this.getAllItems();
        const item = memoBuffer.find(i => i.id === id);
        if (item) {
            if (item.branchName) {
    ...
    getFormattedMemo(currentBranch) {
...
class MemoItem extends vscode.TreeItem {
    constructor(entry, index, currentBranchName) {
